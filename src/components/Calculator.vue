<template>
  <div class="border border-blue-200 rounded-2xl w-full max-w-lg md:px-2 md:py-2">
    ...
    <button class="w-full mt-6 bg-[#00174F] text-white font-semibold py-3 rounded-lg text-lg hover:bg-blue-800 transition">
      Get Started
    </button>
...
const currencies = [
  { code: 'AUD', name: 'Australian Dollar', flag: 'https://flagcdn.com/au.svg', rate: 0.68 },
...
function handleSendAmountChange(e) {
  ...
  if (!isNaN(num) && num >= 0) {
    sendAmount.value = v
    const usd = num * selectedSendCurrency.value.rate
    receiveAmount.value = (
      selectedReceiveCurrency.value.rate
        ? (usd / selectedReceiveCurrency.value.rate)
        : 0
    ).toFixed(2)
...
function handleReceiveAmountChange(e) {
  ...
  if (!isNaN(num) && num >= 0) {
    ...
    const usd = num * selectedReceiveCurrency.value.rate
    sendAmount.value = (
      selectedSendCurrency.value.rate
        ? (usd / selectedSendCurrency.value.rate)
        : 0
    ).toFixed(2)
...
const filteredSendCurrencies = computed(() =>
  currencies.filter(c =>
    c.code.toLowerCase().includes(sendSearch.value.toLowerCase())
  )
)
const filteredReceiveCurrencies = computed(() =>
  currencies.filter(c =>
    c.code.toLowerCase().includes(receiveSearch.value.toLowerCase())
  )
)
...
const exchangeRateText = computed(() =>
  sendAmount.value
    ? `${selectedSendCurrency.value.code} → ${selectedReceiveCurrency.value.code}: ${(
